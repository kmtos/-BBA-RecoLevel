...
*/
  double genDiTauDR = sqrt( (tau1Ref->eta() - tau2Ref->eta())*(tau1Ref->eta() - tau2Ref->eta())  +  (tau1Ref->phi() - tau2Ref->phi())*(tau1Ref->phi() - tau2Ref->phi()) );
  double bestBPt = 0;
  ...
  if (bestBPt > 80 && genDiTauDR <= .5)
  {
    DiTaudR_Ptgt80_->Fill(genDiTauDR );
    std::cout << "\t\tBPt > 80: diTaudR= " << genDiTauDR << std::endl;
  }//if Pt > 80
  if (bestBPt > 100 && genDiTauDR <= .5)
  {
...
void BBAAnalyzer::beginJob()
{
  ...
  DiTaudR_Ptgt80_       = new TH1F("DiTaudR_Ptgt80",     "", 50, 0, .5);
  DiTaudR_Ptgt100_       = new TH1F("DiTaudR_Ptgt100",     "", 50, 0, .5);
